...
`.trim()
  ...
  let id = m.chat
    if (id in conn.math) return conn.reply(m.chat, '⚠️ Masih ada pertanyaan yang belum terjawab di chat ini', conn.math[id][0])
    let math = genMath(mode)
    conn.math[id] = [
        await conn.reply(m.chat, `▢ BERAPA HARGANYA ADALAH *${math.str}*=\n\n_Waktu:_ ${(math.time / 1000).toFixed(2)} detik\n\n🎁 Hadiah : ${math.bonus} XP`, m),
        math, 4,
        setTimeout(() => {
            if (conn.math[id]) conn.reply(m.chat, `⏳ Waktunya habis!\nJawabannya adalah : *${math.result}*`, conn.math[id][0])
      delete conn.math[id]
        }, math.time)
    ]
}
...
handler.tags = ['game']
handler.command = ['mates', 'mtk', 'matemáticas', 'math'] 
