...
▢ *TOP ${len} XP* 🧬
anda : *${usersExp.indexOf(m.sender) + 1}* dari *${usersExp.length}*
...
▢ *TOP ${len} DIAMOND💎* 
anda : *${usersLim.indexOf(m.sender) + 1}* dari *${usersLim.length}*

${sortedLim.slice(0, len).map(({ jid, diamond }, i) => `*${i + 1}.* ${participants.some(p => areJidsSameUser(jid, p.id)) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} ➭ _*Diamond ${diamond}*_`).join`\n`}

▢ *TOP ${len} LEVEL* ⬆️
anda : *${usersLevel.indexOf(m.sender) + 1}* dari *${usersLevel.length}*
